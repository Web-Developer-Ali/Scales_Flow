...
export default function ViewAllDealsClient() {
  ...
  const fetchSuggestions = async (searchQuery: string) => {
    ...
    try {
      const response = await fetch(`/api/deals_search_suggestions?q=${encodeURIComponent(searchQuery)}&limit=10`)
      const data = await response.json()
  ...
  const sortedDeals = [...filteredDeals].sort((a: any, b: any) => {
    ...
    return new Date(b.expectedCloseDate || 0).getTime() - new Date(a.expectedCloseDate || 0).getTime()
  })

  const handleExport = () => {
    // CSV export logic
    const csvContent = [
      ["Company", "Title", "Contact Person", "Value", "Stage", "Probability", "Closing Date"],
          ...
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-foreground">
                {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : metrics.totalDeals}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-foreground">
                {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : `$${(metrics.pipelineValue / 1000).toFixed(0)}K`}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-emerald-400">
                {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : `$${(metrics.closedValue / 1000).toFixed(0)}K`}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-foreground">
                {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : `${Math.round(metrics.avgProbability || 0)}%`}
              </p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Search by company, title, or contact person..."
                  ...
                  <div className="absolute z-10 w-full mt-1 bg-popover border border-border rounded-md shadow-lg max-h-60 overflow-auto">
                    {searchSuggestions.map((suggestion: any, idx: number) => (
                      <div
                        ...
                        onClick={() => {
                          setSearchTerm(suggestion.company || suggestion.title || "")
                          setSuggestions([])
                        }}
                      >
                        <div className="font-medium">{suggestion.company} - {suggestion.title}</div>
                        <div className="text-xs text-muted-foreground">
                          {suggestion.contactPerson && `Contact: ${suggestion.contactPerson}`}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Export Button */}
              <Button onClick={handleExport} className="bg-black hover:bg-black/80 text-white gap-2">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Contact Person</th>
                    ...
                    <tbody>
                      {sortedDeals.map((deal: any) => (
                        <tr key={deal.id} className="border-b border-border hover:bg-secondary/30 transition-colors">
                          ...
                          <td className="py-4 px-4 text-muted-foreground">{deal.title}</td>
                          <td className="py-4 px-4 text-muted-foreground">{deal.contactPerson || '-'}</td>
